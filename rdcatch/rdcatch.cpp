...
//
//      $Id: rdcatch.cpp,v 1.120 2008/10/02 18:52:12 fredg Exp $
//
  ...
  :QWidget(parent,name)
{
  ...
  //
  catch_lock=new RDInstanceLock(QString().sprintf("%s/.rdcatchlock",
						 (const char *)RDHomeDir()));
  if(!catch_lock->lock()) {
    fprintf(stderr,"rdcatch: multiple instances not allowed\n");
    exit(1);
  }
  ...
  // CAE Connection
  //
  catch_cae=new RDCae(this,"catch_cae");
  ...
  catch_cae->connectHost("localhost",CAED_TCP_PORT,catch_config->password());
...
void MainWidget::RefreshList()
{
  ...
  sql=QString("select RECORDINGS.DESCRIPTION,RECORDINGS.IS_ACTIVE,\
               ...
               FEEDS.KEY_NAME from RECORDINGS left join CUTS\
  ...
  //  9 - REC.WED               27 - REC.SWITCH_OUTPUT
				 ...
				 RDRecording::ExitCode code)
{
  ...
  switch(code) {
      case RDRecording::Ok:
	item->setText(30,tr("OK"));
	break;
	
      case RDRecording::Short:
	item->setText(30,tr("Short Length"));
	item->setBackgroundColor(EVENT_ERROR_COLOR);
	break;
	
      case RDRecording::LowLevel:
	item->setText(30,tr("Low Level"));
	item->setBackgroundColor(EVENT_ERROR_COLOR);
	break;
	
      case RDRecording::HighLevel:
	item->setText(30,tr("High Level"));	
	item->setBackgroundColor(EVENT_ERROR_COLOR);
	break;
	
      case RDRecording::Downloading:
	item->setText(30,tr("Downloading"));
	break;
	
      case RDRecording::Uploading:
	item->setText(30,tr("Uploading"));
	break;
	
      case RDRecording::RecordActive:
	item->setText(30,tr("Recording"));
	break;
	
      case RDRecording::PlayActive:
	item->setText(30,tr("Playing"));
	break;
	
      case RDRecording::Waiting:
	item->setText(30,tr("Waiting"));
	break;
	
      case RDRecording::ServerError:
	item->setText(30,tr("Server Error"));
	item->setBackgroundColor(EVENT_ERROR_COLOR);
	break;
	
      case RDRecording::InternalError:
	item->setText(30,tr("Internal Error"));
	item->setBackgroundColor(EVENT_ERROR_COLOR);
	break;
	
      case RDRecording::Interrupted:
	item->setText(30,tr("Interrupted"));
	item->setBackgroundColor(EVENT_ERROR_COLOR);
	break;
	
      case RDRecording::DeviceBusy:
	item->setText(30,tr("Device Busy"));
	item->setBackgroundColor(EVENT_ERROR_COLOR);
	break;

      case RDRecording::NoCut:
	item->setText(30,tr("No Such Cart/Cut"));
	item->setBackgroundColor(EVENT_ERROR_COLOR);
	break;
  }
}
