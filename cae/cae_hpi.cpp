...
//
//      $Id: cae_hpi.cpp,v 1.35 2008/07/18 20:06:10 fredg Exp $
//
...
bool MainObject::hpiUnloadRecord(int card,int stream)
{
#ifdef HPI
...
void MainObject::hpiGetOutputPosition(int card,unsigned *pos)
{
  ...
  for(int i=0;i<RD_MAX_STREAMS;i++) {
    if(play[card][i]==NULL) {
      ...
      pos[i]=((unsigned long long)play[card][i]->currentPosition());
    }
  }
#endif  // HPI
  return;
