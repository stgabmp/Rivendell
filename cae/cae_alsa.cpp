...
//
//      $Id: cae_alsa.cpp,v 1.41.2.3 2010/01/23 02:53:52 cvs Exp $
//
...
{
  int avg_periods=
    (330*rd_config->sampleRate())/(1000*rd_config->alsaPeriodSize());
...
bool MainObject::alsaPlaybackPosition(int card,int stream,unsigned pos)
{
  ...
  unsigned offset=
    (unsigned)((double)alsa_play_wave[card][stream]->getSamplesPerSec()*
  ...
  if(alsa_offset[card][stream]>(int)alsa_play_wave[card][stream]->getSampleLength()) {
    return false;
  }
  alsa_output_pos[card][stream]=0;
...
{// pos is in miliseconds
  ...
  for(int i=0;i<RD_MAX_STREAMS;i++) {
    if (!alsa_play_format[card].exiting){
      pos[i]=((unsigned long long)(alsa_offset[card][i]+
...
bool MainObject::AlsaStartCaptureDevice(QString &dev,int card,snd_pcm_t *pcm)
{
  ...
  if(alsa_play_format[card].sample_rate>0) {
    ...
    sr=rd_config->sampleRate();
  }
  snd_pcm_hw_params_set_rate_near(pcm,hwparams,&sr,&dir);
  if((sr>(rd_config->sampleRate()-RD_ALSA_SAMPLE_RATE_TOLERANCE))&&
     (sr<(rd_config->sampleRate()+RD_ALSA_SAMPLE_RATE_TOLERANCE))) {
    sr=rd_config->sampleRate();
  }
...
void MainObject::EmptyAlsaInputStream(int card,int stream)
{
  ...
  if(alsa_record_wave[card][stream]->getSamplesPerSec()!=
     alsa_capture_format[card].sample_rate) {
    double ratio=(double)alsa_record_wave[card][stream]->getSamplesPerSec()/
      (double)alsa_capture_format[card].sample_rate;
    n=Resample(alsa_wave_buffer,alsa_resample_buffer,
	       n/(2*alsa_input_channels[card][stream]),ratio,
	       alsa_input_channels[card][stream])*
      2*alsa_input_channels[card][stream];
    alsa_record_wave[card][stream]->writeWave(alsa_resample_buffer,n);
  }
  else {
    alsa_record_wave[card][stream]->writeWave(alsa_wave_buffer,n);
  }
}
...
void MainObject::FillAlsaOutputStream(int card,int stream)
{
  ...
  double ratio=(double)alsa_play_format[card].sample_rate/
  ...
  int n=alsa_play_wave[card][stream]->readWave(alsa_wave_buffer,free);
  if(n!=free) {
    alsa_eof[card][stream]=true;
  }
  if(alsa_play_wave[card][stream]->getSamplesPerSec()!=
     alsa_play_format[card].sample_rate) {
    n=Resample(alsa_wave_buffer,alsa_resample_buffer,
	       n/(2*alsa_output_channels[card][stream]),ratio,
	       alsa_output_channels[card][stream])*
      2*alsa_output_channels[card][stream];
    alsa_play_ring[card][stream]->write((char *)alsa_resample_buffer,n);
  }
  else {
