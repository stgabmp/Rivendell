...
//
//      $Id: edit_cart.cpp,v 1.69.2.4 2010/01/20 22:22:50 cvs Exp $
//
    ...
    while(it->current()) {
      if (it->current()->isSelected()) {  
	    ...
	    rdcart_cart_medit->setGroupName(rdcart_group_box->currentText());
          ...
          if(!rdcart_controls.publisher_edit->text().stripWhiteSpace().isEmpty())
            rdcart_cart_medit->setPublisher(rdcart_controls.publisher_edit->text());
          if(!rdcart_controls.composer_edit->text().stripWhiteSpace().isEmpty())
            rdcart_cart_medit->setComposer(rdcart_controls.composer_edit->text());
          if(!rdcart_controls.user_defined_edit->text().stripWhiteSpace().isEmpty())
            rdcart_cart_medit->setUserDefined(rdcart_controls.user_defined_edit->text());
          if(!rdcart_usage_box->currentText().stripWhiteSpace().isEmpty())          
            rdcart_cart_medit->
	      setUsageCode((RDCart::UsageCode)(rdcart_usage_box->currentItem()-1));
          if(!rdcart_cut_sched_box->currentText().stripWhiteSpace().isEmpty())          
            rdcart_cart_medit->
