...
//
//      $Id: voice_tracker.cpp,v 1.77.2.1.2.3 2010/05/11 13:06:21 cvs Exp $
//
...
void VoiceTracker::track1Data()
{
  ...
  rdcae->loadRecord(edit_input_card,edit_input_port,edit_track_cut->cutName(),
...
void VoiceTracker::recordData()
{
  ...
  edit_wave_name[1]=RDCut::pathName(edit_track_cut->cutName());
  wpg[1]=new RDWavePainter(edit_wave_map[1],edit_wave_name[1]);
  wpg[1]->end();
...
{
  if(track_loaded&&
     ((edit_logline[1]->type()==RDLogLine::Cart)||edit_track_cart!=NULL)) {
    ...
    printf("removing cart %06u\n",edit_track_cart->number());
    edit_track_cart->remove();
...
void VoiceTracker::editAudioData()
{
  ...
  RDEditAudio *edit=
    new RDEditAudio(rdcart,edit_logline[edit_rightclick_track]->cutName(),
		    rdcae,rduser,edit_output_card,edit_output_port,
		    edit_tail_preroll,edit_threshold_level,this);
  if(edit->exec()!=-1) {
    rdcart->updateLength();
...
void VoiceTracker::stateChangedData(int id,RDPlayDeck::State state)
{
  ...
  if((id==0)&&(state==RDPlayDeck::Finished)&&
     (edit_deck_state==VoiceTracker::DeckTrack1)) {
    // Initial playout timed out, so back out of the recording
    edit_track_cart->remove();
    delete edit_track_cart;
    edit_track_cart=NULL;
  }
...
void VoiceTracker::recordUnloadedData(int card,int stream)
{
  ...
  track_recording=false;
  if(!track_aborting) {
    edit_track_cut->checkInRecording(rdstation_conf->name());
    ...
    edit_logline[1]->
      setFadeupPoint(edit_track_cut->startPoint(),RDLogLine::LogPointer);
    edit_logline[1]->setFadeupGain(RD_FADE_DEPTH);
    edit_logline[1]->
      setFadedownPoint(edit_track_cut->endPoint(),RDLogLine::LogPointer);
...
//	edit_track_line[2]=line;
    ...
    //
    for(unsigned i=0;i<3;i++) {
      edit_wave_name[i]=GetCutName(edit_track_line[i]);
      if(!(edit_wave_name[i].isEmpty())) {
        if(wpg[i]!=NULL) {
           delete wpg[i];
        }
	wpg[i]=new RDWavePainter(edit_wave_map[i],edit_wave_name[i]);
	wpg[i]->end();
      }
    }
...
bool VoiceTracker::ImportTrack(RDListViewItem *item)
{
  ...
  RDImportAudio *import=
    new RDImportAudio(edit_track_cut->cutName(),edit_import_path,edit_settings,
		      &metadata,wdata,NULL,rdstation_conf,
  ...
  edit_logline[1]->setOriginDateTime(QDateTime(QDate::currentDate(),
					       QTime::currentTime()));
  edit_logline[1]->setFadeupPoint(edit_track_cut->startPoint(),
				  RDLogLine::LogPointer);
  edit_logline[1]->setFadeupGain(RD_FADE_DEPTH);
  edit_logline[1]->setFadedownPoint(edit_track_cut->endPoint(),
				    RDLogLine::LogPointer);
  edit_logline[1]->setFadedownGain(RD_FADE_DEPTH);
  edit_logline[1]->setEndPoint(-1,RDLogLine::LogPointer);
  edit_logline[1]->setSegueEndPoint(edit_track_cut->endPoint(),
				    RDLogLine::LogPointer);
  edit_logline[1]->setHasCustomTransition(true);
  if(!edit_wave_name[2].isEmpty()) {
    edit_logline[2]->setHasCustomTransition(true);
...
QString VoiceTracker::GetCutName(int line)
{
...
bool VoiceTracker::InitTrack()
{
  ...
  edit_track_cart->setOwner(track_log_event->logName());
