...
//
//      $Id: rdcastmanager.cpp,v 1.7.2.11 2009/09/04 01:29:50 cvs Exp $
//
...
void MainObject::PostEpisode()
{
  ...
  if(gethostname(station_name,PATH_MAX)<0) {
    RDCgiError("Unable to determine station name!");
    Exit(0);
  }
  if(cast_key_name.isEmpty()) {
    RDCgiError("Missing KEY_NAME");
  ...
  RDStation *station=new RDStation(station_name,0);
  if(!station->exists()) {
