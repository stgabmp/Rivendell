...
//
//      $Id: pam_rd.cpp,v 1.7 2008/04/22 19:54:07 grauf Exp $
//
...
#define PAM_RD_DESTROY_SHM (1<<5)
#define PAM_RD_FAIL_DEFAULT_USER (1<<6)
...
static int parse_args(int argc, const char **argv, char **tmp_user)
{
    ...
    for (ctrl = 0; argc-- > 0; ++argv) {
        ...
        else if (!strcasecmp(*argv, "destroy_shm")) {
            /* destroy and release the rivendell shared memory */
            ctrl |= PAM_RD_DESTROY_SHM;
        }
                                    ...
                                    const char **argv)
{
    ...
    /* destroy sysv ipc shm if requested */
    if (ctrl & PAM_RD_DESTROY_SHM) {
        int shm_id = -1;
        shm_id = shmget(RD_METER_SHM_KEY,sizeof(struct RDMeterBlock),0);
        if (shm_id != -1) {
            shmctl(shm_id,IPC_RMID,NULL);
            if (ctrl & PAM_RD_DEBUG) {
                syslog(LOG_DEBUG, "shm key:0x%x destroyed", RD_METER_SHM_KEY);
            }
        }
    }
