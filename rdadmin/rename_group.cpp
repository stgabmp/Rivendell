...
//
//      $Id: rename_group.cpp,v 1.11 2007/10/04 20:53:30 dmills Exp $
//
...
void RenameGroup::okData()
{
  ...
  sql=QString().sprintf("select NAME from GROUPS where NAME=\"%s\"",
			(const char *)group_newname_edit->text());
  ...
  // Update Cart List
  //
  sql=QString().sprintf("update CART set GROUP_NAME=\"%s\" \
                         where GROUP_NAME=\"%s\"",
			(const char *)group_newname_edit->text(),
  ...
  //
  if(!merging) {
    sql=QString().sprintf("update GROUPS set NAME=\"%s\" where NAME=\"%s\"",
			  (const char *)group_newname_edit->text(),
			  (const char *)group_name_edit->text());
    q=new RDSqlQuery(sql);
    delete q;
    ...
    // Update AUDIO_PERMS
    //
    sql=QString().sprintf("update AUDIO_PERMS set GROUP_NAME=\"%s\"\
                           where GROUP_NAME=\"%s\"",
			  (const char *)group_newname_edit->text(),
			  (const char *)group_name_edit->text());
    q=new RDSqlQuery(sql);
    ...
    // Update USER_PERMS
    //
    sql=QString().sprintf("update USER_PERMS set GROUP_NAME=\"%s\"\
                           where GROUP_NAME=\"%s\"",
			  (const char *)group_newname_edit->text(),
			  (const char *)group_name_edit->text());
    q=new RDSqlQuery(sql);
