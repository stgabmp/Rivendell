...
//
//      $Id: createdb.cpp,v 1.177.2.29 2010/02/09 21:43:53 cvs Exp $
//
...
bool InitDb(QString name,QString pwd,QString station_name)
{
  ...
  //
  sql="insert into CUTS set CUT_NAME=\"999999_000\",CART_NUMBER=999999,\
  ...
  // Generate Audio
  //
  QString cmd=QString().sprintf("rdgen -t 10 -l 16 %s/999999_000.%s",
...
bool UpdateDb(int ver)
{
  ...
  if(ver<11) {
    q=new QSqlQuery("alter table RDAIRPLAY add column CHECK_TIMESYNC enum('N','Y') default 'Y'");
    delete q;    
