...
//
//      $Id: rdpanel_button.cpp,v 1.21 2007/09/14 14:06:24 fredg Exp $
//
...
void RDPanelButton::setDefaultColor(QColor color)
{
  ...
  button_default_palette=QPalette(color,button_parent->backgroundColor());
}
...
void RDPanelButton::flashButton(bool state)
{
  if(button_flashing&&button_state) {
    if(state) {
      setPalette(QPalette(button_color,button_parent->backgroundColor()));
    }
    else {
      setPalette(QPalette(button_parent->backgroundColor(),
			  button_parent->backgroundColor()));
    }
  }
}
...
void RDPanelButton::WriteKeycap(int secs)
{
  ...
  QBitmap *pix=new QBitmap(size().width(),size().height());
