...
//
//      $Id: rduser.cpp,v 1.23.2.1 2009/04/07 13:59:06 cvs Exp $
//
...
void RDUser::setPassword(const QString &password)
{
  ...
  // Try the "new" syntax first
  //
  sql=QString().sprintf("update USERS set PASSWORD=OLD_PASSWORD(\"%s\") \
                         where LOGIN_NAME=\"%s\"",
			(const char *)password,
			(const char *)user_name);
  q=new RDSqlQuery(sql);
  if(!q->isActive()) {
    ...
    // Then try the "old" syntax
    //
    sql=QString().sprintf("UPDATE USERS SET PASSWORD=PASSWORD(\"%s\") \
                           WHERE LOGIN_NAME=\"%s\"",
			  (const char *)password,
			  (const char *)user_name);
    q=new RDSqlQuery(sql);
