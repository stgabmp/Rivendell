...
//
//      $Id: rdcut_dialog.cpp,v 1.25.2.3 2009/11/25 17:19:24 cvs Exp $
//
...
RDCutDialog::RDCutDialog(QString *cutname,RDStation *station,QString *filter,
			 QString *group,QString username,bool show_clear,
			 bool allow_add,bool exclude_tracks,
  ...
  : QDialog(parent,name,true)
{
  ...
  cut_filter_edit->setValidator(validator);
...
void RDCutDialog::RefreshCarts()
{
  ...
  sql=QString().sprintf("select CART.NUMBER,CART.TITLE,CART.GROUP_NAME,\
                         ...
                         where (%s)&&(CART.TYPE=1)",
	      (const char *)RDCartSearchText(cut_filter_edit->text(),group).utf8());
