...
//
//      $Id: generate_log.cpp,v 1.32.2.1 2009/02/12 14:21:26 cvs Exp $
//
...
void GenerateLog::createData()
{
  ...
  if(log->exists() && cmdswitch==0) {
    ...
    if((tracks=log->completedTracks())>0 && cmdswitch==0) {
...
void GenerateLog::musicData()
{
  ...
  if(((log->linkState(RDLog::SourceMusic)==RDLog::LinkDone)||
     (log->linkState(RDLog::SourceTraffic)==RDLog::LinkDone)) && cmdswitch==0) {
...
void GenerateLog::trafficData()
{
  ...
  QString logname=RDDateDecode(svc->nameTemplate(),gen_date_edit->date());
  RDLog *log=new RDLog(logname);
  if((log->linkState(RDLog::SourceTraffic)==RDLog::LinkDone) && cmdswitch==0) {
    QString str1=QString(tr("The log for"));
