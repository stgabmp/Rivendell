...
//
//      $Id: btacs82.cpp,v 1.9 2008/10/01 13:10:15 fredg Exp $
//
...
BtAcs82::BtAcs82(RDMatrix *matrix,QObject *parent,const char *name)
  : QObject(parent,name)
{
  //
  ...
  //
  bt_istate=0;
  for(int i=0;i<BTACS82_GPIO_PINS;i++) {
...
void BtAcs82::processCommand(RDMacro *cmd)
{
  ...
  switch(cmd->command()) {
	...
	if((cmd->arg(2).toInt()<0)&&(cmd->arg(2).toInt()>=-3)) {
	  lvl=1;
	}
	if((cmd->arg(2).toInt()<-3)&&(cmd->arg(2).toInt()>=-6)) {
	  lvl=2;
	}
	if((cmd->arg(2).toInt()<-6)&&(cmd->arg(2).toInt()>=-10)) {
	  lvl=3;
	}
	if((cmd->arg(2).toInt()<-10)&&(cmd->arg(2).toInt()>=-15)) {
	  lvl=4;
	}
	if((cmd->arg(2).toInt()<-15)) {
	  lvl=5;
	}
